...
class AutonomyLoop:
    ...
    def run(
        ...
        for cycle in range(1, cycle_limit + 1):
            ...
            graph = self.planner.plan(current_goal)
            signature = graph.signature()
            if previous_signature == signature:
                failed_cycles += 1
                self._record_reflection(trace, "strategic", "Repeated plan detected")
            ...
            self._record_reflection(cycle_trace, "operational", f"cycle_{cycle}")

            if not cycle_trace.failed_nodes and self._goal_completed(exit_conditions, cycle_trace):
                self._record_reflection(cycle_trace, "user-preference", "goal-complete")
                self.stop()
                break

            if cycle_trace.failed_nodes:
                failed_cycles += 1
                self._record_reflection(cycle_trace, "self-model", "tool-failure")
                if failed_cycles >= self.max_failed_cycles:
            ...
            self._record_reflection(trace, "strategic", f"cycle_{cycle}_progress")
    ...
    def _record_reflection(self, trace: ExecutionTrace, reflection_type: str, context: str) -> None:
        try:
            reflection = self.reflector.reflect(trace, reflection_type=reflection_type)
