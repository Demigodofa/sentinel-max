...
from sentinel.policy.policy_engine import PolicyEngine, PolicyResult
...
def test_reflection_engine_flags_failures(tmp_path):
    ...
    trace = ExecutionTrace(
        results=[
            ExecutionResult(
                ...
                success=False,
                error="boom",
                policy=PolicyResult(allowed=False, reasons=["Unsafe arguments"]),
            )
        ]
    )
    ...
    assert reflection["policy_outcomes"]["blocked"]
    assert any("policy_blocked" == issue for issue in reflection["issues_detected"])
    assert any(
        "Adjust task" in suggestion for suggestion in reflection["improvement_suggestions"]
    )
