...
def test_full_report_and_health(tmp_path):
    memory = ProjectMemory(storage_path=tmp_path)
    engine = LongHorizonProjectEngine(memory=memory)
    project = engine.create_project(
        name="Health", description="Health check", goals=[{"id": "g1", "text": "Do work"}]
    )

    engine.register_plan(project["project_id"], [{"id": "g1", "action": "do work", "depends_on": []}])
    report = engine.full_report(project["project_id"])

    assert "Health" in report
    assert "No dependency issues" in report

    health = engine.health_report()
    assert "SYSTEM HEALTH" in health
    assert str(tmp_path.resolve()) in health
