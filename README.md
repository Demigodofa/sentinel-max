...
- **GUI startup fallback**: When the display/Tk stack is unavailable (for example, no `DISPLAY` on Linux), the GUI launcher now emits a clear message and automatically falls back to CLI mode so you can still interact with the agent.
- **Windows launcher preflight**: The Windows launcher now checks Tk availability up front and runs a headless Selenium/Chrome preflight so missing Tcl/Tk or Chrome/driver issues are reported before attempting to open windows. GUI blockers still force a CLI fallback, but a failed relay preflight no longer skips the relay — it logs the failure and still launches the Selenium window so you can see the runtime error and driver output.
...
- **ChatGPT browser relay**: Selenium opens a ChatGPT Chrome window (or attaches to an existing debug port), confirms the page is ready, and forwards `<START>...<STOP>` blocks into the GUI queue exactly as if you typed and pressed **Enter**. The Windows launcher starts this by default when `START_BROWSER_RELAY` normalizes to `1`/`true`/`yes`/`on`, logs the normalized flag plus the detected `chromedriver` path, searches the venv and repo directories for a bundled `chromedriver.exe` and prepends it to `PATH` when found, and falls back to Selenium Manager to download a driver when one is not on `PATH` (with a clear warning) instead of silently skipping the relay.
...
- **Conversation router**: `ConversationController` normalizes chat input, routes slash commands, requests confirmation when autonomy is off, and delivers accepted goals to the planner/worker/reflection loop. When `AUTO_MODE` is enabled, natural text is handed straight to the orchestrator so tool calls run without `/run`, with live plan-step updates feeding the GUI plan panel. Slash-command flows (/auto, /tools, etc.) now retain pipeline correlation IDs so telemetry stays linked even when the intent engine is bypassed.
...
- **Direct tool execution + orchestrator**: `/tool <name> <json>` and natural language requests ("list files in sandbox", "search web for ...", `action: <tool>`) now run through a tool-calling orchestrator that invokes the sandboxed registry for real and streams outputs back into the conversation instead of pretending to execute. CLI `/tool` inputs pasted as multiple lines are split and executed in sequence, so batch tool invocations no longer fail on JSON parse errors.
- **Tool registry normalization + telemetry**: ToolRegistry now applies schema-driven argument filtering and aliases (`num_results`/`limit`/`top_k` → `max_results`, `command` → `argv`, `endpoints` → `description`), converts sandbox commands from strings into argv lists, and emits telemetry through an event sink while retrying once when tools reject unexpected keyword arguments. Repairs are persisted to `tool_aliases.json` under the memory directory to automatically drop or remap troublesome parameters on future calls. Unit tests cover these aliases so CLI/GUI payloads that use older parameter names still execute successfully.
...
- `microservice_builder` writes generated FastAPI apps to `SENTINEL_PROJECT_STORAGE/<service>/app.py`, drops a paired `requirements.txt`, returns a ready-to-run `uvicorn` command, and tracks running services by name/port so you can `action: list|stop|restart|logs` without leaking threads.
