...
- **Tool registry normalization + telemetry**: ToolRegistry now applies schema-driven argument filtering and aliases (`num_results`/`limit`/`top_k` → `max_results`, `command` → `argv`, `endpoints` → `description`), converts sandbox commands from strings into argv lists, and emits telemetry through an event sink while retrying once when tools reject unexpected keyword arguments. Unit tests cover these aliases so CLI/GUI payloads that use older parameter names still execute successfully.
