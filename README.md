...
- **Windows launcher preflight**: The Windows launcher now checks Tk availability up front and runs a headless Selenium/Chrome preflight so missing Tcl/Tk or Chrome/driver issues are reported before attempting to open windows. Failed checks switch the launcher to CLI mode or skip the ChatGPT relay with explicit log messages instead of silently hanging.
