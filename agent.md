...
- The Windows launcher (`start_sentinel_max.bat`) starts the ChatGPT browser relay watcher by default. Toggle `START_BROWSER_RELAY=0` in the script to skip starting it; relay startup status is logged to `logs/launcher_last.log` and surfaces in a dedicated window.
